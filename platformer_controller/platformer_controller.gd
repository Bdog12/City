...
@export var climbCost= 2
@export var climbSpeed = 200
@export var WallJumpCost = 100
...
func jump():
	...
	if not is_feet_on_ground() and coyote_timer.is_stopped():
			...
			if current_stamina >0:
				...
				if Input.is_action_just_pressed("space"):
					...
					if $AnimatedSprite2D.flip_h == true:
						...
						velocity.y -= jump_velocity 
						current_stamina -= WallJumpCost
				if Input.is_action_pressed("up") and ! wallJumping:
					velocity.y -= climbSpeed
					current_stamina -=climbCost
...
func StaminaLabel():
	$Control/Label.text = str(current_stamina)



func _on_camare_hit_box_area_exited(area):
	emit_signal("EnterNewCamareZone")
